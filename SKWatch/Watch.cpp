...
bool CWatch::Load(SString sPathFile)
{
	...
	for(iModule=0;iModule<iModules;iModule++)
	{
		...
		pNew->iSn = iModule;
		pNew->iStartErrTimes = pNew->iStartTimes = 0;
		pNew->tLastStartErr = pNew->tLastStart = -1;
	...
	for(m=0;m<cnt;m++)
	{
		...
		if(!bRun)
		{
			...
			else
			{
				...
				pExtModule->is_run = true;
				pExtModule->iStartTimes++;
				pExtModule->tLastStart = ::time(NULL);
			}
		}		
	}
	m_Lock.unlock();
